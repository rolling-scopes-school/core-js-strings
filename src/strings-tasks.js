...
function sumOfCodes(str) {
  if (str && str.length > 0) {
    return str.split('').reduce((sum, symb) => sum + symb.charCodeAt(0), 0);
  }
  return 0;
}
...
function countVowels(str) {
  ...
  return str
    .split('')
    .reduce((count, char) => count + (vowels.has(char) ? 1 : 0), 0);
}
...
function findLongestWord(sentence) {
  let longestWorld = '';
  sentence.split(/\s/).forEach((world) => {
    if (world.length > longestWorld.length) {
      longestWorld = world;
    }
  });
  return longestWorld;

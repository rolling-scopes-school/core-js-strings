...
function removeFirstOccurrences(str, value) {
  ...
  if (f >= 0) return str.slice(0, f) + str.slice(f + value.length, str.length);
  return str;
}
...
function getStringFromTemplate(firstName, lastName) {
  const w = `Hello, ${firstName} ${lastName}!`;
  return w;
}

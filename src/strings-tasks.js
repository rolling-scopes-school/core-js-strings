...
function removeFirstOccurrences(str, value) {
  ...
  const str1 = str.slice(0, indexValue);
  const str3 = indexValue + value.length;
  const str2 = str.slice(str3);
...
function removeLastOccurrences(str, value) {
  ...
  const str1 = str.slice(0, indexValue);
  const str2 = indexValue + value.length;
  return str1.concat(str.slice(str2));
}
...
function isPalindrome(str) {
  const str1 = str.toLowerCase().trim().replaceAll(/\W/g, '');
...
function findLongestWord(sentence) {
  ...
  let word = '';
  for (let i = 0; i < arr.length; i += 1) {
    if (arr[i].length > lengist) {
      lengist = arr[i].length;
      word = arr[i];
    }
  }
  return word;
...
function extractNameFromTemplate(value) {
  const str1 = value.search(',');
  const str2 = value.search('!');
  return value.substring(str1 + 1, str2).trimStart();
}

...
function sumOfCodes(str) {
  ...
  return Array.from(str).reduce((arr, char) => arr + char.charCodeAt(0), 0);
}
...
function findLongestWord(sentence) {
  const clearedSentence = sentence.trim();
  const words = clearedSentence.split(' ');
  return words.reduce((long, word) =>

...
function sumOfCodes(str) {
  ...
  return str
    .split('')
    .map((ch, indx) => str.charCodeAt(indx))
...
function countVowels(str) {
  return [...str].filter((x) => [...'aeiouyAEIOUY'].includes(x)).length;
}
...
function isPalindrome(str) {
  const pattern = /((\w))/g;
  const ss = str.toLowerCase().match(pattern);
...
function reverseWords(str) {
  const arr = str.split(' ');
  const revArr = arr.map((elt) => [...elt].reverse().join(''));
  return revArr.join(' ');
}

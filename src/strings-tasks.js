...
function isPalindrome(str) {
  const lowercaseStr = str.toLowerCase().match(/[a-z]/g).join('');
  return lowercaseStr === reverseString(lowercaseStr);
}
...
function extractNameFromTemplate(value) {
  ...
  return arr
    .map((item) =>
      item === arr[arr.length - 1] ? item.substr(0, item.length - 1) : item
    )
    .join(' ');
}

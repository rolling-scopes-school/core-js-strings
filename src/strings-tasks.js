...
function isPalindrome(str) {
  const strLowercase = str
    ...
    .replaceAll('!', '')
    .replaceAll(',', '');
  const lengthStr = strLowercase.length;
  if (lengthStr % 2 === 0) {
    const middle = lengthStr / 2;
    const firstPartsStr = strLowercase.slice(0, middle);
    const secondPartsStr = strLowercase.slice(middle);
    const reverseSecondPartsStr = secondPartsStr.split('').reverse().join('');
    if (reverseSecondPartsStr === firstPartsStr) {
      return true;
    }
  }
  if (lengthStr % 2 === 1) {
    const middle = (lengthStr - 1) / 2;
    const firstPartsStr = strLowercase.slice(0, middle);
    const secondPartsStr = strLowercase.slice(middle + 1);
    const reverseSecondPartsStr = secondPartsStr.split('').reverse().join('');
    if (reverseSecondPartsStr === firstPartsStr) {
      return true;
    }
...
function invertCase(str) {
  const invertedCaseStr = str.split('').map((char) => {
    if (char.toLowerCase() === char) {
      return char.toUpperCase();
    }
    return char.toLowerCase();
  });
  return invertedCaseStr.join('');
}
...
function encodeToRot13(str) {
  const alphabeticals = `ABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLM
  abcdefghijklmnopqrstuvwxyzabcdefghijklm?!           ?!`.split('');
  let rot13 = '';
  for (let i = 0; i < str.length; i += 1) {
    rot13 +=
      str[i] === ' ' ? ' ' : alphabeticals[alphabeticals.indexOf(str[i]) + 13];
  }
  return rot13;
}

...
function sumOfCodes(str) {
  if (!str) {
    return 0; // Handle null, undefined, or empty string
  }
...
function isPalindrome(str) {
  ...
  // Check if the second half matches the popped characters from the stack
  for (
    let i = Math.ceil(cleanedStr.length / 2);
    i < cleanedStr.length;
    i += 1
  ) {
...
function extractNameFromTemplate(value) {
  ...
  // If a match is found, extract the name from the first capturing group
  if (match) {
    return match[1];
  }
  return '';
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (char >= 'A' && char <= 'Z') {
      const newCharCode =
        ...
        'A'.charCodeAt(0);
      encodedStr.push(String.fromCharCode(newCharCode));

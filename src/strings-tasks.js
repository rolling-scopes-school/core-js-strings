...
function removeFirstOccurrences(str, value) {
  ...
  // console.log(leng);
  const result = str.slice(indexOfSub, indexOfSub + leng);
  // console.log(result);
  const res = str.replace(result, '');
  return res;
}
...
function removeLastOccurrences(/* str, value */) {
  throw new Error('Not implemented');
  // const LastOccurrencesIndex = str.lastIndexOf(value);
  // // console.log(LastOccurrencesIndex);
  // const lengthOfValue = value.length;
  // // console.log(lengthOfValue);
  // const result = str.slice(
  //   LastOccurrencesIndex,
  //   LastOccurrencesIndex + lengthOfValue
  // );
  // // console.log(result);
  // const res = str.replace(result, '');
  // // console.log(res);
...
function sumOfCodes(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const codeOfEveryChar = str[i].charCodeAt(0);
    sum += codeOfEveryChar;
  }
  return sum;
}
...
function countVowels(str) {
  ...
  let count = 0;
  for (let i = 0; i < str.length; i += 1) {
    if (vowelsLetters.indexOf(str[i]) !== -1) {
      count += 1;
    }
  }
  return count;
...
function isPalindrome(str) {
  const strRemoved = str
    .toLowerCase()
    .replaceAll(' ', '')
    .replaceAll(',', '')
    .replaceAll('!', '')
    .replaceAll('?', '');
  const newStrReverse = str
    .toLowerCase()
    .replaceAll(' ', '')
    .replaceAll(',', '')
    .replaceAll('!', '')
...
function findLongestWord(sentence) {
  ...
  // console.log(arr);
  arr.map(function iteration(item) {
...
function extractNameFromTemplate(value) {
  const valueTemp = value.replaceAll('!', '');
